...
def write_module_start(f, year):
    suffix = "_" + year
    f.write("module ciaaw__capi_saw" + suffix + newline)
    f.write("    !! Codata Constants - Autogenerated: C API."+newline)
    f.write("    use iso_c_binding, only: c_null_char, c_int" + newline)
    f.write("    use ciaaw__saw" + suffix + newline)
    f.write("    use ciaaw__capi_saw_type" + newline)
    f.write("    private"+newline)
    f.write(newline)

def write_year(f, year):
    suffix = get_suffix(year)
    f.write(f"integer(c_int), protected, public, bind(C, name=\"ciaaw_saw_YEAR{suffix}\") :: capi_YEAR{suffix:s} = YEAR{suffix:s} !capi" + newline)
    f.write(newline)
...
def write_module_end(f, year):
    suffix = "_" + year 
    f.write("end module ciaaw__capi_saw" + suffix)
