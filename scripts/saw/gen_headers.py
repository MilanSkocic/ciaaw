...
def write_module_start(f, year):
    suffix = "_" + year
    f.write(f"#ifndef CIAAW_SAW{suffix:s}_H" + newline)
    f.write(f"#define CIAAW_SAW{suffix:s}_H" + newline)
    ...
    f.write("#include \"ciaaw_saw_type.h\"" + newline)
    f.write(newline)

def write_year(f, year):
    suffix = get_suffix(year)
    f.write(f"ADD_IMPORT extern const int ciaaw_saw_YEAR{suffix:s};" + newline)
    f.write(newline)

def write_constant(f, symbol, year):
    suffix = get_suffix(year)
    f.write(f"ADD_IMPORT extern const struct ciaaw_saw_type ciaaw_saw_{symbol:s}{suffix:s};")
    f.write(newline)
